...
const print = async () => {
  if (gameSaved.value) {
    window.open(`${ENDPOINTS.printer.relative}/${id.value}?showAnswers=1`, "_blank"); // Open print page in a new tab
  } else {
...
<template>
  <main>
    ...
    <input type="text" id="title-input" name="title-input" v-model="creatorStore.title" @input="handleInput"
      @focus="userIsTyping = true" @blur="userIsTyping = false" ref="titleInput"
    ...
    <input type="checkbox" name="highlight-checkbox" id="highlight-checkbox" v-model="creatorStore.highlight"
      @change="boardRef.toggleHighlights()" :disabled="loadingStore.isLoading" />
    <label for="highlight-checkbox">Kuva peidetud sõnad</label>
    <button @click="generate" :disabled="loadingStore.isLoading">Genereeri</button>
    <button @click="share" :disabled="!generated || gameSaved || loadingStore.isLoading">Jaga mängu</button>
    <template v-if="gameSaved">
      <input type="text" id="link-field" name="link-field" readonly :value="link" ref="linkFieldRef"
        :disabled="loadingStore.isLoading" />
      <button @click="copyLink" :disabled="loadingStore.isLoading">{{ linkCopied ? "Kopeeritud" : "Kopeeri" }}</button>
    </template>
    <button @click="print" :disabled="!generated || loadingStore.isLoading">Prindi mäng</button>
